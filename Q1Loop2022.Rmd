...
```{r} 
setwd('~/Desktop/PID/Campaign Finance Data')

#Q1 Oppose
OpposeTaxHike <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question1/Coalition_to_Stop_the_Tax_Hike_Amendment.xlsx")

#Q1 Support
CoalitionforSocialJusticeFairShare <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question1/CoalitionforSocialJusticeFairShare.xlsx")

Fair_Share_Massachusetts <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question1/Fair_Share_Massachusetts.xlsx") 

#Q2 Oppose
`Committee For Competitive Dental Plans for Consumers` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question2/Committee For Competitive Dental Plans for Consumers.xlsx")
`Committee to Protect Access to Quality Dental Care` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question2/Committee_to_Protect_Access_to_Quality_Dental_Care.xlsx")

#Q2 Support
`Massachusetts Dental Care Providers for Better Dental Benefits` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question2/Massachusetts Dental Care Providers for Better Dental Benefits.xlsx")
`Committee on Dental Insurance Quality` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question2/Committee on Dental Insurance Quality.xlsx")

#Q3 
`21st_Century_Alcohol_Retail_Reform_Committee` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question3/21st_Century_Alcohol_Retail_Reform_Committee.xlsx")

#Q4
`Fair_And_Secure_Massachusetts` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question4/Fair_And_Secure_Massachusetts.xlsx")

`Vote_YES_for_Work_and_Family_Mobility` <- read_xlsx("~/Desktop/PID/BallotQuestions2022/Question4/Vote_YES_for_Work_and_Family_Mobility.xlsx")
```
...
Q2022 <- lapply(Q12022, function(df) {
...
2. The proportion of the number of all contributions for each variable of `Category_PID`. This is labelled `Proportion_Of_All_Contributions`.
...
```{r} 
print_summary_table <- function(df_name, df) {
  print(paste("Creating table for:", df_name))  # Debugging statement
  summary_df <- df %>%
    filter(Category_PID != "Refund") %>%
    group_by(Category_PID) %>%
...
```{r} 
for (i in seq_along(Q2022)) {
  table <- table(names(Q2022)[i], Q2022[[i]])
  filename <- paste0("table_", names(Q2022)[i], ".html")
