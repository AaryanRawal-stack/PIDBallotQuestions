...
##DATA WRANGLING
# Filter out unwanted records from each list
```{r}
# For Q2022
Q2022 <- lapply(Q2022, function(df) {
  df %>% 
    filter(`Record Type Description` != "Aggregated Unitemized Receipts" &
           `Record Type Description` != "Aggregated Unitemized Inkind Contributions" &
           `Record Type Description` != "Bank Interest")
})
...
classified_tables_Q2020 <- lapply(Q2020, classification)

for (df_name in names(classified_tables_Q2020)) {
  cat("Table Name:", df_name, "\n")
  print(create_summary_table(df_name, classified_tables_Q2020[[df_name]]))
}

classified_tables_Q2018 <- lapply(Q2018, classification)

for (df_name in names(classified_tables_Q2018)) {
  cat("Table Name:", df_name, "\n")
  print(create_summary_table(df_name, classified_tables_Q2018[[df_name]]))
}
```
...
export_summary_table_png <- function(summary_table, file_name) {
  #Place File
  output_dir <- "~/Desktop/PID/Campaign_Finance_Data"
...
for (df_name in names(classified_tables_Q2020)) {
  cat("Table Name:", df_name, "\n")
  summary_table <- create_summary_table(df_name, classified_tables_Q2020[[df_name]])
  export_summary_table_png(summary_table, file_name = df_name)
}

for (df_name in names(classified_tables_Q2018)) {
  cat("Table Name:", df_name, "\n")
  summary_table <- create_summary_table(df_name, classified_tables_Q2018[[df_name]])
  export_summary_table_png(summary_table, file_name = df_name)
}
```
